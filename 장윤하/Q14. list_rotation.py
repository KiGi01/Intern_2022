...
a = input("input=").split(',')  # input의 값을 ','로 구분하여 저장

num = int(a.pop(0))  # a의 0번째 값을 찾아 num에 저장하고 그 값을 제거

b = a[-num:] + a[:-num]  # a의 뒤에서 num번째 값까지, a의 뒤에서 num번째 값을 제외한 값까지 순서대로 저장
print(','.join(b))
...
b = a[-num:] + a[:-num] # a의 앞에서 -num번째 값부터 끝까지, a의 앞에서 num번째 이전 값까지 순서대로 저장
print(','.join(b))