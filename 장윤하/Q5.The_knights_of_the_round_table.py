...
a, b, c = map(float, input("삼각형 세 변의 길이를 입력하세요: ").split())     # a, b, c에 각각 세 변의 길이 저장

s = (a + b + c) / 2                                     # 헤론의 공식을 이용
S = math.sqrt(s * (s - a) * (s - b) * (s - c))          # s(s-a)(s-b)(s-c)의 제곱근 계산
print(round(S, 3))                                      # 삼각형의 넓이를 소수 셋째 자리에서 반올림
...
=============================================================================
"""
r = (2 * S) / (a + b + c)               # 원은 삼각형에 내접할 때 가장 큼 r(a+b+c) / 2 = S
print(round(r, 3))                      # 반지름을 소수 셋째 자리에서 반올림