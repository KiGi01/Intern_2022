...
for i in range(alx, arx):                       # 두 직사각형의 x좌표 확인
   for j in range(blx, brx):
        if i == j:                              # 만약 겹친다면, 1 더하기
            same_x += 1

same_y = 0                                      # same_y = 직사각형이 겹치는 y좌표 (시작은 0)
for i in range(aly, ary):                       # 두 직사각형의 y좌표 확인
   for j in range(bly, bry):
        if i == j:                              # 만약 겹친다면, 1 더하기
            same_y += 1
...
def is_triple(rectangle_a, rectangle_b, rectangle_c):
    ...
    same_x = 0                                      # same_x = 직사각형이 겹치는 x좌표 (시작은 0)
    for i in range(alx, arx):                       # 세 직사각형의 x좌표 확인
        for j in range(blx, brx):
            for x in range(clx, cly):
                if i == j == x:                     # 만약 겹친다면, 1 더하기
                    same_x += 1

    same_y = 0                                      # same_y = 직사각형이 겹치는 y좌표 (시작은 0)
    for i in range(aly, ary):                       # 세 직사각형의 y좌표 확인
        for j in range(bly, bry):
            for x in range(cly, crx):
                if i == j == x:                     # 만약 겹친다면, 1 더하기
                    same_y += 1
