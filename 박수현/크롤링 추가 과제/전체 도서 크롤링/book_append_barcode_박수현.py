...
if __name__ == '__main__':
    ...
    # 2.기존 바코드 파일 리스트로 변환하기
    for i in range(2):
        category_name = ["유아(0~7세)", "어린이(초등)"]
        with open(f"book_barcode_{category_name[i]}.txt", "r") as f:
            current_list = f.read().splitlines()
        
        # 3. 목록 진입 및 바코드 추출 프로그램 실행하기
        append_list = is_book_barcode(i)

        # 4. 이전 리스트와 비교하여 추가된 바코드만 새로운 파일에 저장하기
        book_barcode = list(set(append_list) - set(current_list))
        print(len(book_barcode))

        with open(f"book_barcode_append_{category_name[i]}.txt", "w", encoding="UTF-8") as f:
