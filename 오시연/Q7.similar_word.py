...
def OneEditApart(s1, s2):
    if len(s1) == len(s2):        #문자열 길이 같을 경우
        for i in range(len(s1)):  #문자열 길이만큼 반복
            j = s1[:i]+s1[i+1:]   #i번째 인덱스를 제외한 문자열을 변수에 담았을 때 두 문자열이 같아지는지 확인
            k = s2[:i]+s2[i+1:] 
            if j == k:
                return True
        return False
        ...
        for i in range(len(s1)): #s1 길이만큼 반복
            j = s1[:i]+s1[i+1:]  #i번째 인덱스를 제외한 문자열을 변수에 담고 같아지는지 확인
            if j == s2:
                return True
        return False
    elif len(s1) < len(s2):      #s2에서 문자 1개를 제거했을 때 s1과 같아지는지 확인
        for i in range(len(s2)): #s2 길이만큼 반복
            k = s2[:i] + s2[i+1:] #첫글자 제외 담음(ats), 두번째글자 제외 담음(cts), 세번째글자 제외 담음(cas), 끝글자 제외 담음(cat)
            if s1 == k:   #동일할 때 
                return True
        return False
