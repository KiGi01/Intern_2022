...
a, b, c = triangle[0], triangle[1], triangle[2]   #하나씩 변수에 저장
s = (a+b+c)/2       
result = math.sqrt(s*(s-a)*(s-b)*(s-c)) #세변의 길이를 이용한 삼각형의 넓이 구하는 공식. sqrt:제곱근 반환
...
a, b, c = triangle2[0], triangle2[1], triangle[2]
s = (a+b+c)/2  
r= math.sqrt((2*(s-a)*(s-b)*(s-c))/(a+b+c))   #세변의 길이를 이용한 내접원의 반지름 구하는 공식
