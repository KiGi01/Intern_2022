...
def count_palindrome(num, cnt = 0):   #함수 정의. 매개변수 cnt의 기본값을 0으로 지정
    if num == int(str(num)[::-1]):    #전달된 값을 슬라이스를 이용해 뒤집었을 때의 수와 비교하여 회문인지 판단
        print(cnt, num)               #조건 만족할시 출력
        return                        #함수 종료
    else:
        count_palindrome(num + int(str(num)[::-1]), cnt + 1)  #원래 수에 뒤집은 수를 더한 수, 반복횟수를 인수로 재귀호출

input_ = int(input('입력: '))      #정수 형태로 입력받음
count_palindrome(input_)           #함수 호출
...
def palindrome(num):               #반복횟수를 저장할 변수를 제외한 함수
    if num == int(str(num)[::-1]): 
        print(num)
        return       
    else:
        palindrome(num + int(str(num)[::-1]))  

input2_ = int(input('입력 개수: '))
input_nums = list(map(int,input('정수 입력: ').split(', '))) #콤마 기준 분리, map을 이용해 정수로 변환, 리스트로 반환
for i in range(input2_):       #입력 개수만큼 반복
    palindrome(input_nums[i])  #인덱스로 요소에 순서대로 접근, 함수 호출
