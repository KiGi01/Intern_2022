...
#os.walk: 지정된 디렉터리부터 그 아래의 모든 하위 디렉터리와 파일까지 차례대로 접근
for (root, directories, files) in os.walk(dir_path): #파일 목록, 하위 폴더 내의 파일 목록에 접근
...
for (root, dir, files) in os.walk(dir_path): #os.walk 모든 하위 디렉토리와 파일에 접근
    for filename in files:  
        if '.txt' in filename: 
                f = open(os.path.join(root, filename),'r')  #디렉토리 경로에 파일 이름을 조합, open으로 읽기 모드로 열기
