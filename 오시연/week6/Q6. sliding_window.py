...
window_size = 3
num_list = ['1','3','-1','-3','5','3','6','7']
max_list, min_list = [], [] #최대값과 최소값 넣을 리스트
window = list(map(int, num_list)) #숫자 리스트를 정수 리스트로 변환
last_index = len(window) -3   #리스트의 길이 - 윈도우의 크기(윈도우가 리스트 범위를 벗어나지 않도록)
for i in range(last_index + 1): #리스트 범위를 벗어나지 않는 수까지 반복
      max_list.append(max(window[i:i+3])) #윈도우 크기만큼 슬라이스하여 가져온 값 중 최대값을 추가
      min_list.append(min(window[i:i+3]))  #윈도우 크기만큼 슬라이스하여 가져온 값 중 최소값을 추가
print('최대값: ', *max_list, sep=' ') #압축을 풀고 공백으로 구분하여 출력
print('최소값: ', *min_list, sep= ' ')
