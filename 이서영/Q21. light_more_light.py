...
n = tuple(map(int, input('input: ').split(',')))

for x in n : 
    if x != 0 :                     # x가 0이 아닐 경우
        light = False               # light는 꺼져있는 상태
        for i in range(1, x+1) :    # 1부터 x까지의 범위까지 반복
            if x%i == 0 :           # x가 i로 나누어 떨어질때 
                light = not light   # ligth는 not light(켜짐)
        if light :                  # light일 경우(켜져있을 경우), yes 출력
            print("yes")
        else :                      # not light일 경우(꺼져있을 경우), no 출력
            print("no")
    else :                          # x가 0일 경우
        break                       