...
num = input('input = ')
re_num = num[::-1]                      # re_num은 num의 회문
count = 0                               # 회문을 발견하기 위해 반복한 횟수

while(re_num != num) :                  # num과 re_num이 같을 때까지 반복
    num = str(int(num) + int(re_num))   
    re_num = num[::-1]
    count += 1                          # count 값에 1더하기

print('output =', count, num)           # 반복 횟수와 회문 출력
...
n = int(input('input = ')) # 입력받을 정수의 수
numbers = list(input('입력받을 정수 = ').split(', ')) # input 값을 ', '구분자로 나눈 후, 리스트로 묶기

for num in numbers :                    # number리스트 요소들을 차례대로 num 변수에 대입
    re_num = num[::-1]                  # re_num은 num의 회문
    count = 0                           # 회문을 발견하기 위해 반복한 횟수
    while(re_num != num) :              # num과 re_num이 같을 때까지 반복
        num = str(int(num) + int(re_num))
