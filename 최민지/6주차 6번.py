...
"""
sliding_size = 3
list1 = [1, 3, -1, -3, 5, 3, 6, 7]

def sliding_window(sliding_size, list1):
    min_list = [] #최솟값을 넣을 빈 리스트
    max_list = [] #최댓값을 넣을 빈 리스트
    for i in range(len(list1) - sliding_size + 1): #리스트에서 슬라이딩윈도우의 크기만큼 뺀 길이만큼 반복
        window = []
        for j in range(sliding_size): #슬라이딩윈도우의 길이만큼 반복
            window.append(list1[i+j]) #리스트에 넣는다.
        min_list.append(min(window)) #최솟값이 나왔을 시. 최솟값을 리스트에 넣고
        max_list.append(max(window)) #최댓값이 나왔을 시, 최댓값을 리스트에 넣기

    return min_list, max_list

print(sliding_window(sliding_size, list1)[1])
print(sliding_window(sliding_size, list1)[0])