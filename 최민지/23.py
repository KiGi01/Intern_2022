...
for num in range (a):
    n = int(str(a)[::-1]) #끝까지 -1칸 간격(역순으로) 뒤집어진 숫자로부터 시작.
    for i in range (1, 1000): #1000번 미만
        n = n + int(str(n)[::-1]) #n과 뒤집어진 n이 더해진다.
        if n == int(str(n)[::-1]): #만약 n과 뒤집어진 n이 같다면
            break #조건문이 종료
...
input_1 = (3)
numbers = input("입력받을 정수를 적으세요: ").split(", ") #', '을 기준으로 입력

for num in range (numbers):
    n = int(str(numbers)[::-1]) #끝까지 -1칸 간격(역순으로) 뒤집어진 숫자로부터 시작.
    for input_1 in range (1, 1000):
        n = n + int(str(n)[::-1]) #n은 n과 뒤집어진 n을 더한 값
        if n == int(str(n)[::-1]): #만약 뒤집어진 n과 n이 같다면
            break #조건문을 종료
