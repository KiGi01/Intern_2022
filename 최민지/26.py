...
과제 1. 아서 왕의 방 안에 존재하는 삼각형 모양 창문의 세 변의 길이가 주어질 때, 창문의 넓이를 구하시오.
       - 조건 : 삼각형의 넓이는 소수 셋째 자리에서 반올r림 하시오.
...
a, b, c =map(float, input("삼각형 세 변의 길이를 입력하세요 : ").split())
s = float((a+b+c)/2)

A = (s*(s-a)*(s-b)*(s-c))**(1/2)

print("The radius of the round table is : {}".format(round(A,3))) #r(반지름)을 소수 셋째 자리에서 반올림한다. round = 반올림, format = {} 안에 r이 들어간다.
...
a, b, c =map(float, input("삼각형 세 변의 길이를 입력하세요 : ").split())
s = float((a+b+c)/2) 
A = (s*(s-a)*(s-b)*(s-c))**(1/2) #루트는 **(1/2)로 표현, 삼각형 넓이(A) = '루트 (s*(s-a)(s-b)(s-c))'로 헤론공식 이용
r = 2*A/(a+b+c) #(a+b+c)*r/2 = 삼각형넓이(A)

print("The radius of the round table is : {}".format(round(r,3))) #r(반지름)을 소수 셋째 자리에서 반올림한다. round = 반올림, format = {} 안에 r이 들어간다.