...
def total(a):
    b = int(a.pop(0)) #a를 지워준다.
    list = a[-b:]+a[:-b] #a[-b:]는 마지막 번호, a[:-b]는 마지막 번호는 b를 제외한 번호 나열(a는 삭제처리해서 없음)
    return ", ".join(list) #공백을 넣고 리스트를 만든다. a b c

a = input("첫값은 회전량, 나머지는 문자열 입력:").split(" ") #split은 문자와 같이 나누는 것 ex) Hello -> H e l l o
print(str(total(a)))