...
def find_palindrome(n):
    cnt = 1                                       #반복 횟수
    while cnt<1000 :
        reverse_n  = int(str(n)[::-1])            #591 (n 뒤집은 수)
        n = str(n +reverse_n) #786                # n+ n 뒤집은 수 ; str
        cnt2 = 0                                    # 회문 판별용
        for j in range(len(n)//2):                  # n의 길이를 2로 나눈 몫만큼 반복 
            if n[j] ==n[-(j+1)]:                    # 양쪽 대칭 자리값이 동일한지
                cnt2 +=1                            # 동일한 경우 +1
        
        if cnt2 == len(n)//2:                 #회문 판단      
            break                             #회문일 경우 break
...
def find_palindrome_list(n):
    userint_list = []
    for i in range(n):           #입력받은 정수만큼 input 반복
        userint = int(input())
        userint_list.append(userint)
        
    for i in userint_list:    
        find_palindrome2(i)



find_palindrome_list(3)
 #195, 265, 750