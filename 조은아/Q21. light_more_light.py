...
def IsLightOn(bulbs_list):
    for bulbs in bulbs_list:
        if bulbs == 0:
            break
        else:
            results = [0]*bulbs #전구수
            for i in range(1, bulbs+1): #마부횟수 
                for j in range(1, bulbs+1): #전구 위치 
                    if j%i == 0:
                        results[j-1] += 1
#                print(results)
            if results[-1]%2 : #1홀수 yes
                print("yes")
            else:
                print("no")
